...
const nextConfig = {
  ...
  async redirects() {
    return [
      {
        source: '/:path*',
        has: [{ type: 'host', value: 'www.amplixy.com' }],
        destination: 'https://amplixy.com/:path*',
        permanent: true,
      },
    ]
  },
  async headers() {
    ...
    const scriptSrc = `'self' 'unsafe-inline'${isDev ? " 'unsafe-eval'" : ''} https://trafic.amplixy.cloud https://app.cal.com`
    return [
      {
        source: '/:path*',
        headers: [
